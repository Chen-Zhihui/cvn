...
bool Pnet::init(const std::string & filename){
    Pthreshold = 0.6;
    nms_threshold = 0.5;
    firstFlag = true;
    this->rgb = new pBox;
...
bool Rnet::init(const std::string & filename){
    Rthreshold = 0.7;
...
bool Onet::init(const std::string & filename){
    Othreshold = 0.8;
    this->rgb = new pBox;
...
mtcnn::mtcnn(int row, int col, 
    ...
    for(int i = 0; i < scales_.size(); i++) {
        simpleFace_[i].init(pfile);
    }
    refineNet.init(rfile);
    outNet.init(ofile);
}
...
std::vector<struct Bbox> mtcnn::findFace(Mat &image){
    struct orderScore order;
    int count = 0;
    ...
    if(count<1)return vector<struct Bbox>();
