...
// 2-norm of matrix = largest singular value of mat.
                                            ...
                                            const cv::Mat & xy, int option_k = 2)
        {
            ...
            // We know that X * r = U
            auto rnk = rank(X);
            poco_assert( rnk >= 2 * K); // unique solution TODO: SVD, then solve
        ...
        cv::Mat cp2tform(const FPoints & src, const FPoints & dst, cp2tform_type type) {
            poco_assert(src.size() == dst.size() && "Number of src facial points and dst facial points don't match.");
            ...
            cv::Mat trans;
            switch (type) {
                case cp2tform_similarity :
                    return move(findNonreflectiveSimilarity(src_mat, dst_mat));
                case cp2tform_noreflective:
        ...
        cv::Mat align(const cv::Mat & sample, const std::vector<cv::Point> & fpts,
                      const std::vector<cv::Point> & ref_fpts, cp2tform_type type) {
            cv::Mat tform, img;
            tform = cp2tform(fpts, ref_fpts, type).colRange(0,2).t();
            cv::warpAffine(sample, img, tform, sample.size());
            return img;
        }
    }
}